...
argv = minimist process.argv.slice(2),
	...
	boolean: 'h help v version examples w overwrite z compress P pretty l list s size verify info c colors C compat Q verbose debug q quiet stdout'.split ' '
	default:
		root: '/'
...
#{pkg.name}
Examples:
create archive from dir:            #{pkg.name} dir archive
same with named parameters:         #{pkg.name} -i dir -o archive
extract archive to dir:             #{pkg.name} archive dir
extract root from archive to dir:   #{pkg.name} archive dir -r root
extract d/file from archive to dir: #{pkg.name} archive dir -r d/file
verify archive:                     #{pkg.name} archive --verify
list archive entries:               #{pkg.name} archive -l
list archive entries for root:      #{pkg.name} archive -l -r root
list entries for root with pattern: #{pkg.name} archive -l -r root -p pattern
list archive entries with size:     #{pkg.name} archive -ls
print content of file to stdout:    #{pkg.name} archive -r d/file --stdout
	"""
...
# we have at least an input
	...
	if verify
	...
	else if output or stdout
		# transcode in -> out
		usageError 'output and --stdout not allowed together' if stdout and output
		try
		...
		if inputStat.isDirectory()
			# create archive
			writeTo = output or process.stdout
			usageError 'using --root is not allowed for packing' if root isnt '/' # this should be allowed (for appending at least)
			console.log "packing #{inputPath} to #{(output or '<stdout>').info}" unless quiet
			asar.createArchive input, writeTo, pattern, done
		else
			# extract archive
			usageError 'extracting to stdout not supported' if stdout
